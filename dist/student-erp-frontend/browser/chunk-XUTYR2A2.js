...
// src/app/core/listing.util.ts
var LIST_PAGE_SIZE_OPTIONS = [10, 25, 50, 100];
...
function applyDefaultListSort(listSort, keys) {
  for (const key of keys) {
    if (!listSort[key] && DEFAULT_LIST_SORT[key]) {
      listSort[key] = __spreadValues({}, DEFAULT_LIST_SORT[key]);
    }
  }
}
...
function sortItems(items, field, direction, accessors) {
  if (!field || !accessors[field])
    return [...items];
...
function exportRowsToPdf(title, headers, rows) {
  ...
  const schoolName = document.querySelector('meta[name="app-school-name"]')?.content || document.title || "School ERP";
  const bodyRows = rows.map((row, i) => `<tr${i % 2 ? ' class="alt"' : ""}>${row.map((cell) => `<td>${cell}</td>`).join("")}</tr>`).join("");
    ...
    <style>
      *{box-sizing:border-box}
      ...
      <div class="doc-header">
        ...
        <span class="meta">${title}<br>Exported ${(/* @__PURE__ */ new Date()).toLocaleString()}</span>
      </div>
...
//# sourceMappingURL=chunk-XUTYR2A2.js.map
