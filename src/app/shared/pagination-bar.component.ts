  ...
  template: `
    <footer class="pagination-bar" *ngIf="totalItems > 0">
      ...
      <div class="pagination-controls">
        ...
        <div class="pagination-nav" *ngIf="showNavigation">
          <button
            type="button"
            class="ghost small pagination-nav-btn"
            [disabled]="currentPage <= 1"
            (click)="changePage(currentPage - 1)"
            aria-label="Previous page"
          >
            <span class="pagination-nav-icon" aria-hidden="true">&#8249;</span>
        ...
        <div class="pagination-page-nums" *ngIf="totalPages > 1">
          <button type="button" class="ghost small" [disabled]="currentPage <= 1" (click)="changePage(1)" title="First page" aria-label="First page">&laquo;</button>
          <ng-container *ngFor="let p of visiblePages">
            <button type="button" class="ghost small page-num" [class.active-page]="p === currentPage" (click)="changePage(p)" [attr.aria-current]="p === currentPage ? 'page' : null">{{ p }}</button>
          </ng-container>
          <button type="button" class="ghost small" [disabled]="currentPage >= totalPages" (click)="changePage(totalPages)" title="Last page" aria-label="Last page">&raquo;</button>
        </div>
      </div>
    </footer>
  `
